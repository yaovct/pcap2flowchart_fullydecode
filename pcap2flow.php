...
?>
	...
	<form id="pkt_clicked" method="POST" style="display: none">
		<input type="text" id="packet_no" name="packet_no" class="form-control" value="" />
		<input type="text" id="file_info" name="file_info" class="form-control" value="" />
		<input type="text" id="packet_info" name="packet_info" class="form-control" value=""/>
	</form>
</div>
...
$(function() {
	$('area').on('click',function(event) {
		event.preventDefault(); // 避免回到原來的頁面

		// 開啟於不同視窗
		var xwin = "FullDecodeWin" + Math.ceil(Math.random()*1000).toString();
		window.open("", xwin ,"width=1024,height=750,toolbar=no,scrollbars=yes,resizable=yes");
		$('#packet_no').val($(this).data('id')+1);
		$('#file_info').val('<?=$input_filename?>');
		$('#packet_info').val($(this).data('packet_info'));

		$('#pkt_clicked')
		  .attr('action', "packet_info.php")
		  .prop('target', xwin)
		  .submit();
